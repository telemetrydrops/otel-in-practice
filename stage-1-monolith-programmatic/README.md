# E-commerce Monolith (Temporary Stage — Programmatic SDK)

A temporary teaching stage that configures the OpenTelemetry Go SDK directly in code. The application and its instrumentation match `stage-1-monolith`; only the SDK bootstrap differs.

## Features

- REST API for users, products, and orders
- PostgreSQL database integration
- Programmatic OpenTelemetry SDK setup for traces, metrics, and logs
...
OpenTelemetry is configured in `internal/telemetry/setup.go`. The setup builds one shared resource and the three signal providers:

- `TracerProvider` with an OTLP/HTTP exporter, batch processor, parent-based ratio sampler, and span limits
- `MeterProvider` with an OTLP/HTTP exporter, periodic reader, two views of the order-duration histogram, and trace-based exemplars
- `LoggerProvider` with an OTLP/HTTP exporter and batch processor

All exporters send to the local LGTM stack on `http://localhost:4318`. `resource.Default()` also reads standard resource environment variables, so attributes not fixed by this example can be supplied with `OTEL_RESOURCE_ATTRIBUTES`.

This stage is intentionally verbose enough to expose the SDK primitives. For new applications, prefer the declarative configuration used by `stage-1-monolith`.
